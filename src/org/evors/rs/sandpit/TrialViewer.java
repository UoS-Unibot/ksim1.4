...
public class TrialViewer extends SandPitCanvas implements Runnable
{
	...
	private boolean simulationLoaded = false; // whether a simulation has been
												// loaded.
	private SimulatedKJunior robot; // robot to render.
	private PathTracer path; // path of robot
	public static long DELAY = Math.round(1000 / 60); // delay in milliseconds
														// between frames.
	private volatile boolean simulationStopped = false; // whether the
														// simulation has been
														// stopped.
	 ...
	 *
	 * @param controller
	 *            RunController of simulation to load
	 */
	 ...
	 *            Graphics context.
	 * @param strings
	 *            Array of strings; each element is a single line.
	 */
	public void drawText(Graphics2D g2, String[] strings)
	{
		...
		g2.fillRect((int) x1, (int) y - fm.getHeight(), dx + 5, fm.getHeight() * strings.length + 5);
	...
	public void draw()
	{
		...
		{
			controllerStr = "Neurons: " + ((CTRNN) controller.getController()).getNeurons().toString();
		} else
		...
		drawText(g2, new String[] { "Time: " + timeF.format(time), "Robot pos: " + posF.format(robot.getPosition().x) + ", " + posF.format(robot.getPosition().y),
				"Heading: " + headF.format(robot.getPolarOrientation()), "Input: " + EvoRSLib.arrayToStringInputs(robot.getInput()), controllerStr });
