...
public class KJuniorPhilRegressionTest extends TestCase  {
	...
	double FullIRval(Vec2 p1,double a, Wall wall)
	{
		...
		p = IntersectionPhilRegression.PhilIntersection_Point(p1, p2, wall.start, wall.end);  // find intersection point mid ray to wall along ray
		d=dist(p1,p);
	//printf("d; %f   ",d); debug
		return(IRval(d));
	}
	...
	boolean ray_hit(Vec2 p1, double a1, double a2)
	{
		...
		{
			if(IntersectionPhilRegression.PhilIntersect(p1,p2,world[i].start,world[i].end)) // line segmentp1,p2 is ray, does intersect with a wall?
				iflag=true;
			i++;
		}
	...
	Ray_HitNearestResult ray_hitNearest(Vec2 p1, Vec2 p2)
	{
		...
		{
			if(IntersectionPhilRegression.PhilIntersect(p1,p2,world[i].start,world[i].end)) // line segmentp1,p2 is ray, does intersect with the wall?
				{iflag=true;
				p = IntersectionPhilRegression.PhilIntersection_Point(p1, p2, world[i].start, world[i].end); // find intersection point
				 dl = dist( p, p1 );
